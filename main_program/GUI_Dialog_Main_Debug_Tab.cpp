...
wxString CheckForNameClashes( void(*)(void), long unsigned );    // defined lower down in this file
...
#define PRINT_TYPE_LEN(type, len) print_size.operator()<type>( wxS( #type ), len )
    ...
    s += wxS("\nChecking for hash collisions in the authors' names:\n");
    ...
    s += CheckForNameClashes( []() {  wxGetApp().SafeYield(nullptr, false); }, 800u );
...
wxString CheckForNameClashes( void (*const callback)(void), long unsigned const interval )
{
    ...
    auto Item = [](std::size_t i) -> std::pair< std::uint_fast64_t, wxString >
        {
            if ( i < count_primary ) return { std::get<0u>(g_primary_names_unsorted[i]), std::get<1u>(g_primary_names_unsorted[i]) };
            i -= count_primary;
            return { std::get<0u>(g_alternative_names_unsorted[i]), std::get<1u>(g_alternative_names_unsorted[i]) };
        };

    std::size_t const total_count = count_primary + count_alternative;

    long unsigned counter = 0u;
    for ( std::size_t i = 0u; i < (total_count - 1u); ++i )
    {
    ...
    if ( retval.empty() ) retval = wxS(" -- no hash collisions detected --\n");
    return retval;
}
